...
<script>
	...
	function checkn(input){
		var value = $(input).val();
		var reg = /^[A-Za-z]{1}[A-Za-z0-9_]{5,14}$/g;
		if(value==""){
			$("#sp1").html("用户名不能为空！");
	...
	function checkp(input){
		...
		if(value==""){
			$("#sp2").html("密码不能为空！");
			$("#sp2").css({"font-size":"14px","color":"#be3131"});
		}else{
			var flag=reg.test(value);
			if(flag){
				$("#sp2").html("✔");
				$("#sp2").css({"font-size":"18px","color":"#228B22"});
			}else{
				$("#sp2").html("格式不正确");
				$("#sp2").css({"font-size":"14px","color":"#be3131"});
			}
	...
	function checkp1(input){
		...
		if(value==""){
			$("#sp3").html("密码不能为空！");
			$("#sp3").css({"font-size":"14px","color":"#be3131"});
		}else{
			if(value==val){
				$("#sp3").html("✔");
				$("#sp3").css({"font-size":"18px","color":"#228B22"});
			}else{
				$("#sp3").html("两次密码不一致！");
				$("#sp3").css({"font-size":"14px","color":"#be3131"});
			}
		...
		if(value==""){
			...
			$.ajax({
				...
				success:function(result){
					if("用户名已存在！"==result){
						$("#sp5").html("✔");
						$("#sp5").css({"font-size":"18px","color":"#228B22"});
					}else{
						//$("#sp5").html("该用户不存在");
						//$("#sp5").css({"font-size":"14px","color":"#be3131"});
					}
	...
	function checki(input){
		...
		$.ajax({
			...
			success:function(mg){
				if("验证码错误"==mg){
					$("#sp7").html("✘");
					$("#sp7").css({"font-size":"18px","color":"#be3131"});
				}else{
					$("#sp7").html("✔");
					$("#sp7").css({"font-size":"18px","color":"#228B22"});
				}
	...
	//切换验证码图片的方法
	function changeImg(){
		$("#code").attr("src","imgCode.do?r="+Math.random());
	}
</script>
<div id="bodyPart">		
	<!--头部导航开始-->
           ...
           <form action="register.do" method="post">
           	<div class="user_form"> 
                <div class="uInfo">
                    ...
                    <input type="text" class="text" name="login_name" onblur="checkn(this)"/>
                    <span id="sp1"></span>
                </div>
                <div class="uInfo">
                	<span class="label">设置密码：</span>
                    <input type="password"  class="text" name="password"  onblur="checkp(this)" id="pass"/>
                    <span id="sp2"></span>
                </div> 
                <div class="uInfo">
                	<span class="label">确认密码：</span>
                    <input type="password" class="text"  onblur="checkp1(this)"/>
                    <span id="sp3"></span>
                </div>
                <div class="uInfo">
                	<span class="label">邮箱：</span>
                    <input type="text" class="text"  name="email" onblur="checke(this)"/> 
                    <span id="sp4"></span> 
                </div> 
                <div class="uInfo">
                	<span class="label">推荐人用户名：</span>
                    <input type="text" class="text"  name="recommender" onblur="checkt(this)"/>
                    <span id="sp5"></span>
                </div>
                <div class="uInfo">
                	<span class="label">验证码：</span>
                    <input type="text"  onblur="checki(this)"/><img id="code" src="imgCode.do" /><font>看不清？</font><a><span id="sp6" onclick="changeImg()">换一张</span></a>
                    <span id="sp7"></span>
                </div>
                <div class="uInfo">
                	<span class="label">&nbsp;</span>
                    <input type="submit" id="button" value="同意以下协议，提交"/><span>${msg}</span>
                </div>
            </div>     
            </form>
